<template>
  <div class="space-y-6">
    <div>
      <h3 class="text-2xl font-bold text-gray-900 mb-2 text-center">Upload Bank Statements</h3>
      <p class="text-gray-600 text-center">This is needed to verify your application. You can upload PDF files.</p>
    </div>
    ...
    <div class="space-y-4">
      <div v-for="(statement, index) in bankStatements" :key="index" class="space-y-2">
        ...
        <div class="relative">
          <input :ref="el => { if (el) fileInputs[index] = el as HTMLInputElement }" type="file"
            accept=".pdf,.jpg,.jpeg,.png" @change="handleFileChange(index, $event)" class="hidden" />
          <div @click="triggerFileInput(index)" @dragover.prevent="handleDragOver(index)"
            ...
            <div class="flex flex-col items-center justify-center text-center">
              <svg v-if="!statement.file" class="w-12 h-12 text-gray-400 mb-3" fill="none" stroke="currentColor"
                viewBox="0 0 24 24">
              ...
              <button v-if="statement.file" @click.stop="removeFile(index)" type="button"
    ...
    <div class="bg-yellow-50 border-2 border-yellow-200 rounded-lg p-4 flex items-start space-x-3">
      ...
      <div class="text-sm text-yellow-900">
        ...
        <p>Please upload at least your most recent bank statement. Additional statements help speed up the approval
          process.</p>
      </div>
    </div>
  </div>
